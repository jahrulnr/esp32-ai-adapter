...
constexpr const char* kDefaultSttModel = "openai/gpt-4o-mini-transcribe";
constexpr const char* kDefaultTtsModel = "openai/gpt-4o-mini-tts";
...
String parseOpenRouterErrorCode(const SpiJsonDocument& doc) {
...
String parseOpenRouterErrorMessage(const SpiJsonDocument& doc) {
...
void addOpenRouterOptionalHeaders(const String& httpReferer,
                                  const String& appTitle,
                                  AiHttpRequest& outHttpRequest) {
  if (httpReferer.length() > 0) {
    outHttpRequest.addHeader("HTTP-Referer", httpReferer);
  }
  if (appTitle.length() > 0) {
    outHttpRequest.addHeader("X-OpenRouter-Title", appTitle);
  }
}

}  // namespace

bool OpenRouterAudioAdapter::supportsSpeechToText() const {
  return true;
}

bool OpenRouterAudioAdapter::supportsTextToSpeech() const {
  return true;
}

bool OpenRouterAudioAdapter::supportsSpeechToTextStreaming() const {
  return true;
}

bool OpenRouterAudioAdapter::supportsTextToSpeechStreaming() const {
  return false;
}

bool OpenRouterAudioAdapter::buildSpeechToTextRequest(const AiSpeechToTextRequest& request,
                                                      AiHttpRequest& outHttpRequest,
...
bool OpenRouterAudioAdapter::parseSpeechToTextResponse(const AiHttpResponse& httpResponse,
...
bool OpenRouterAudioAdapter::buildTextToSpeechRequest(const AiTextToSpeechRequest& request,
                                                      AiHttpRequest& outHttpRequest,
                                                      String& outErrorMessage) const {
  if (request.apiKey.length() == 0) {
    outErrorMessage = "Missing OpenRouter API key";
...
bool OpenRouterAudioAdapter::parseTextToSpeechResponse(const AiHttpResponse& httpResponse,
  ...
  if (!isSuccess) {
    outResponse.ok = false;
    outResponse.errorCode = parseOpenRouterErrorCode(doc);
    outResponse.errorMessage = parseOpenRouterErrorMessage(doc);
    return false;
  }
